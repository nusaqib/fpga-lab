# Module 22: RFDC bring-up + DAC->ADC loopback on RFSoC4x2, tile-2 pair
# (DAC_A = DAC tile 230 block 0 / vout20, ADC_A = ADC tile 226 / vin2_23 -
# tile/channel mapping and every tile parameter below mirror the official
# PYNQ base overlay for this board, RFSoC-PYNQ boards/RFSoC4x2/base/base.tcl,
# trimmed to just the tile-2 pair).
#
# Datapath: dc_source (constant I=0.5FS) -> DAC fine mixer (NCO makes the
# carrier) -> SMA -> SMA -> ADC fine mixer (NCO brings it near DC) ->
# axis_snap BRAM recorder -> AXI4-Lite -> A53. The PS also programs the
# RFDC itself (s_axi) and, before any of this works, the LMK/LMX clock
# chips over PS SPI0 (that part is pure software, see src/rfclk.c).
